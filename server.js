...
async function handleLogin(req, res) {
  ...
  try {
    ...
    // ─── [LOGIN-ALL] Populate rawAllStudents by looping all departments x priorities 1-5.
    //     The bare GetDepartmentApplicationSummary call above returns [] without params,
    //     so we fetch it per-department the same way /api/placement/popular does.
    try {
      const [deptRes2, acYearRes2, semRes2, yearRes2, termRes2] = await Promise.all([
      ...
      const departments2 = JSON.parse(deptRes2.body || '{}').data || [];
      const acYears2 = JSON.parse(acYearRes2.body || '{}').data || [];
      const semesters2 = JSON.parse(semRes2.body || '{}').data || [];
      const years2 = JSON.parse(yearRes2.body || '{}').data || [];
      const terms2 = JSON.parse(termRes2.body || '{}').data || [];

      const acYear2 = acYears2[0] ? acYears2[0].AcYear : '2025/2026';
      const semester2 = semesters2[0] ? semesters2[0].Semester : 2;
      const year2 = years2[0] ? years2[0].Year : 1;
      const term2 = terms2[0] ? terms2[0].Term : 'II';

      function sleepLogin(ms) {
        return new Promise(function (resolve) { setTimeout(resolve, ms); });
      }

      function fetchDeptLogin(d) {
            ...
            .then(function (r) {
              ...
              return arr.map(function (row) {
                return {
                  ...
                  FullName: row.FullName || row.fullName || ((row.FirstName || '') + ' ' + (row.FatherName || '')).trim(),
                  DestinationDepartment: deptName,
                  Priority: prio,
                  TotalResult: row.TotalResult || row.TotalScore || '',
                  ApplicationStatus: row.ApplicationStatus || '',
                  PlacementStatus: row.PlacementStatus || '',
                  HighschoolExam: row.HighschoolExam || '',
                  NonExamTotalResult: row.NonExamTotalResult || '',
                  NoneExamTotalResult: row.NoneExamTotalResult || '',
                  Exam: row.Exam || '',
                  ExamResult: row.ExamResult || '',
                  Gender: row.Gender || '',
                  AcademicStatus: row.AcademicStatus || '',
                  AcademicStanding: row.AcademicStanding || '',
                  AcademicResult: row.AcademicResult || '',
                  AcYear: row.AcYear || '',
                  AcademicYear: row.AcademicYear || '',
                  Semester: row.Semester || '',
                  Year: row.Year || '',
                  AcademicYearShort: row.AcademicYearShort || '',
      ...
      const BATCH = 3;
      console.log('[LOGIN-ALL] fetching', departments2.length, 'departments x 5 priorities...');
      const t0 = Date.now();
      for (let i = 0; i < departments2.length; i += BATCH) {
        const batch = departments2.slice(i, i + BATCH);
        const results = await Promise.all(batch.map(fetchDeptLogin));
        results.forEach(function (r) { rawAllStudents.push.apply(rawAllStudents, r); });
        if (i + BATCH < departments2.length) await sleepLogin(200);
      }
      console.log('[LOGIN-ALL] fetched', rawAllStudents.length, 'rows in', ((Date.now() - t0) / 1000).toFixed(1), 's');
    } catch (e) {
      console.error('[LOGIN-ALL] error:', e.message);
    }
