...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('profile-student-id').textContent = bio.studentId || '';
  ...
  html += profileRow('book', 'Program', data.program);
  html += profileRow('building', 'Department', bio.department || 'Freshman');
  html += profileRow('calendar', 'Enrollment', bio.enrollmentDate);
  html += '</div>';
  
  // Personal Info
  html += '<div class="profile-section-title">Personal Info</div>';
  html += '<div class="profile-card">';
  html += profileRow('user', 'Gender', bio.gender);
  html += profileRow('phone', 'Phone', bio.phone);
  html += profileRow('mail', 'Email', bio.email || '—');
  html += profileRow('globe', 'Nationality', bio.nationality);
  html += profileRow('birthday', 'Birth Date', bio.birthDate);
  html += '</div>';
  ...
  function profileRow(icon, label, value) {
    ...
    const icons = {
      ...
      birthday: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>'
    };
    
    return '<div class="profile-row">' +
      '<div class="profile-row-icon">' + (icons[icon] || '') + '</div>' +
      '<div><div class="profile-row-label">' + label + '</div>' +
      '<div class="profile-row-value">' + value + '</div></div>' +
      '</div>';
  }
});
